...
kxy_news={
	guanggao:function(){
		...
		news_list:function(){
			var state=0;//开始页数
			if (getQueryString('pro_duct')==null)
			{
				fenlei=1
				$(".picture_one>span").addClass('nc_meng_show')

			}else{
				fenlei=getQueryString('pro_duct')
			};//分类，classification
			var zong;
			function click(){
			...
			function page(){
				$.ajax({  //根据后台数据条数生成分页按钮
					type:"get",
					url:'http://www.jzzsyy.com:2403/about?{"id":"count","classification":'+fenlei+'}',
			...
			function ajax(){
				$.ajax({  //默认显示的数据
					type:"get",
					url:'http://www.jzzsyy.com:2403/about?{"$skip":'+state+',"$limit":12,"classification":'+fenlei+',"$fields":{"title":1,"date":1}}',
					success:function(data){
						$(".nc_loading").hide()
						$("#list_one").show()
						$("#list_one").html("");
						for(var i in data){
							...
							$("#list_one").append('<li><a href="About_artical.html?id='+data[i].id+'"><span>'+data[i].title+'</span><span class="nc_list_date">'+data[i].date+'</span></a></li>')
						}
					},
					beforeSend:function(){
						$("#list_one").hide()
