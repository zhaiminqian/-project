...
if($lei=="1"){
	...
	$("#picture_six").children('span').addClass('nc_meng_show')

}else if($lei=="2"){
	nc_bluedeg.style.top='198px';
	$(".nc_picture_toggle").children('span').removeClass('nc_meng_show')
	$("#picture_seven").children('span').addClass('nc_meng_show')
}
...
kxy_news={
	guanggao:function(){
		...
		news_list:function(){
			var state=0;//开始页数
			if (getQueryString('lei')==null)
			{
				fenlei=1
				$(".picture_one>span").addClass('nc_meng_show')
			}else{
				fenlei=getQueryString('lei')
			};
//分类，classification
			var zong;
			var ajax_true=false;
			...
			function page(){
				$.ajax({  //根据后台数据条数生成分页按钮
					type:"get",
					url:'http://www.jzzsyy.com:2403/jobs?{"id":"count","classification":'+fenlei+'}',
			...
			function ajax(){
				$.ajax({  //默认显示的数据
					type:"get",
					url:'http://www.jzzsyy.com:2403/jobs?{"$skip":'+state+',"$limit":12,"classification":'+fenlei+',"$fields":{"title":1,"date":1},"$sort":{"sort":1}}',
					success:function(data){
						$(".nc_loading").hide()
						$("#list_one").show()
						$("#list_one").html("");
						for(var i in data){
							...
							$("#list_one").append('<li><a href="rczp_artical.html?id='+data[i].id+'"><span>'+title+'</span><span class="nc_list_date">'+data[i].date+'</span></a></li>')
						}
					},
					beforeSend:function(){
						$("#list_one").hide()
