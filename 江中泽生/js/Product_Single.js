...
single={
    single_hover:function(){
		...
		var myUrl="http://www.jzzsyy.com:2403/products/";		
	...
	//single_carousel:function(){
		...
		function pro_play_ajax(){
			$.ajax({
			   type:'GET',
			   url:'http://www.jzzsyy.com:2403/products?id='+$id+'&classification='+$classfication,
			   dataType:'json',
			   success:function(e){
				  var htmll;
				  var title=e.title;
				  ...
				  var img=e.img;
				  var Costprice=e.Costprice;
				  var content=unescape(e.content);
		...
		for (var i=0;i<single_tab.length;i++)
		{
			...
			single_tab[i].onclick=function(){
				...
				if ($classfication!=fenlei)
				{
					$.ajax({
					   type:'GET',
					   url:'http://www.jzzsyy.com:2403/products?{"classification":'+fenlei+',"$sort":{"sort":-1}}',
					   dataType:'json',
					   success:function(e){
						  if (e=='')
						  {
