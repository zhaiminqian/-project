...
pro_center={
		news_list:function(){
			...
			function page(){
				$.ajax({  //根据后台数据条数生成分页按钮
					type:"get",
					url:'http://www.jzzsyy.com:2403/products?{"id":"count","classification":'+fenlei+',"tosubindex":"1"}',
			...
			function ajax(){
				$.ajax({  //默认显示的数据
					type:"get",
					url:'http://www.jzzsyy.com:2403/products?{"$skip":'+state+',"$limit":3,"classification":'+fenlei+',"tosubindex":1,"$fields":{"summary":1,"img":1,"title":1,"classification":1,"tosubindex":1},"$sort":{"sort":-1}}',
					success:function(data){
						if (data=='')
						{
