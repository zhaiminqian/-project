	...
	search = {
		searchTab:function(){
			...
			$.ajax({
				...
				type:'GET',
				success:function(e){
					$('.search_foot').css('backgroundImage','url(http://www.jzzsyy.com/jz/public/upload/images/'+e[0].img+')')
				}
			})
			...
			function basicAjax(x){
				$.ajax({
					url:'http://www.jzzsyy.com:2403/news?{"title":{"$regex":"'+encodeURIComponent(searchval)+'","$options":"i"},"$skip":'+x+',"$limit":12,"$sort":{"sort":1}}',
					dataType:'json',
					type:'GET',
					async:false, 
					success:function(r){
			...
			function basicAjaxpro(x){
				$.ajax({
					url:'http://www.jzzsyy.com:2403/products?{"title":{"$regex":"'+encodeURIComponent(searchval)+'","$options":"i"},"tosubindex":1,"$skip":'+x+',"$limit":12,"$sort":{"sort":1}}',
					dataType:'json',
					type:'GET',
					async:false, 
					success:function(r){
						$('#searchTabb').html('')
						for(var i = 0;i<r.length;i++){
							$('#searchTabb').append('<li><a href = "Product_Single.html?id='+r[i].id+'&ificaction='+r[i].classification+'"><div></div><img src = "http://www.jzzsyy.com/jz/public/upload/images/'+r[i].img[0].url+'"/><p>'+r[i].title+'</p></a></li>')
						}
					}
				})
			}
			...
			function invokingA(){
				...
				$.ajax({
					url:'http://www.jzzsyy.com:2403/news?{"title":{"$regex":"'+encodeURIComponent(searchval)+'","$options":"i"},"id":"count"}',
					dataType:'json',
					type:'GET',
					success:function(e){
			...
			function invokingB(){
				...
				searchval = search_inp.value;
				$.ajax({
					url:'http://www.jzzsyy.com:2403/products?{"title":{"$regex":"'+encodeURIComponent(searchval)+'","$options":"i"},"tosubindex":1,"id":"count"}',
					dataType:'json',
					type:'GET',
					success:function(e){
			...
			$.ajax({
				url:'http://www.jzzsyy.com:2403/key?{"$sort":{"sort":1}}',
				type:'GET',
				success:function(e){
					// console.log(e)
					$('.search_list').html('')
