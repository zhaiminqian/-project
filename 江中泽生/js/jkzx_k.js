...
if($jianK=="1"){
	...
	$(".nc_picture_toggle").children('span').removeClass('nc_meng_show')
	$("#picture_four").children('span').addClass('nc_meng_show')
}else if($jianK=="3"){
	nc_bluedeg.style.top='346px';
	$(".nc_picture_toggle").children('span').removeClass('nc_meng_show')
	$("#picture_five").children('span').addClass('nc_meng_show')
}
kxy_news={
	guanggao:function(){
		$.ajax({
		...
		news_list:function(){
			...
			function page(){
				$.ajax({  //根据后台数据条数生成分页按钮
					type:"get",
					url:'http://www.jzzsyy.com:2403/info?{"id":"count","classification":'+fenlei+'}',
			...
			function ajax(){
				$.ajax({  //默认显示的数据
					type:"get",
					url:'http://www.jzzsyy.com:2403/info?{"$skip":'+state+',"$limit":12,"classification":'+fenlei+',"$fields":{"title":1,"date":1}}',
					success:function(data){
						$(".nc_loading").hide()
						$("#list_one").show()
						$("#list_one").html("");
						for(var i in data){
							...
							$("#list_one").append('<li><a href="jkzx_artical.html?id='+data[i].id+'"><span>'+title+'</span><span class="nc_list_date">'+data[i].date+'</span></a></li>')
						}
					},
					beforeSend:function(){
						$("#list_one").hide()
