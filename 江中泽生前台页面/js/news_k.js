...
//分类，classification
			...
			function page(){
				$.ajax({  //根据后台数据条数生成分页按钮
					type:"get",
					url:'http://60.205.105.211:2403/news?{"id":"count","classification":'+fenlei+'}',
			...
			function ajax(){
				$.ajax({  //默认显示的数据
					type:"get",
					url:'http://60.205.105.211:2403/news?{"$skip":'+state+',"$limit":12,"classification":'+fenlei+',"$fields":{"title":1,"date":1},"$sort":{"sort":1}}',
					success:function(data){
						$(".nc_loading").hide()
						$("#list_one").show()
						$("#list_one").html("");
