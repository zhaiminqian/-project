...
//  加载条的拖拽 ：播放条
    ...
    BoF();
    video.src='http://60.205.105.211/zhuanti/Home-page-video/jzzs_officeVideo_1.mp4?t='+Math.random();
...
Homepage.AjaxZ=function(){
	$.ajax({
		url:'http://60.205.105.211:2403/news?{"$skip":0,"$limit":4,"$sort":{"sort":-1}}',
		dataType:"json",
		type:"get",
		success:function(e){
			var htmlN='';
			...
			for(var i=0;i<e.length;i++){
                ...
                htmlN2+='<a href="news_artical.html?id='+id+'"><li><div class="news"><div class="opcity_bj"></div><img src="http://60.205.105.211/jz/public/upload/images/'+img+'"><div class="Home_bj">'+title+'</div></div><div class="Home_text"><div class="aa">'+content+'</div></div></li></a>'
	        };
        ...
        //新闻内容截取图片以及字数
	        for(var i=0;i<$('.Home_text').length;i++){
		     	var ht=$('.aa').eq(i).html();
		     	var ht=ht.replace(/<.*?>/ig,"");
		     	if( $(".aa").eq(i).find('img').size()!=0  ){
	           	    $(".aa").eq(i).children('p').children('img').remove();
	           	    $(".aa").eq(i).children('img').remove();
		        }
		        if( ht.length >= 100 ){
	            	var reset_ht=ht.substr(0,100);
	            	$(".aa").eq(i).html(reset_ht+'...');
		        }
	        }
	    }
	});
    $.ajax({
		url:'http://60.205.105.211:2403/products?{"$skip":0,"$limit":6,"toindex":1,"$sort":{"sort":-1}}',
		dataType:"json",
		type:"get",
		success:function(e){
		    	var htmlP=''
				for(var i=0;i<e.length;i++){
		            ...
		            htmlP+='<li><a href="Product_Single.html?id='+id+'&ificaction='+classification+'" style="background:url(http://60.205.105.211/jz/public/upload/images/'+img+') center center no-repeat;background-size:cover"><div class="opcity_bj"></div><div class="Home_bj2">'+title+'</div></a></li>'            
                }
                $(".Home_show").html(htmlP);
		    }
	});
	$.ajax({
		url:'http://60.205.105.211:2403/info?{"$skip":0,"$limit":3,"toindex":1,"classification":3,"$sort":{"sort":-1}}',
		dataType:"json",
		type:"get", 
		success:function(e){
			var htmlI='';
			for(var i=0;i<e.length;i++){
				...
				var img=e[i].img;
				var id=e[i].id;
	            htmlI+='<li><a href="jkzx_artical.html?id='+id+'" style="background:url(http://60.205.105.211/jz/public/upload/images/'+img+') center center no-repeat;background-size:cover"><div class="opcity_bj"></div><div class="Home_bj3">'+title+'</div></a></li>'
	        } 
	        $(".HomeAnli").html(htmlI);
		}
	});
	$.ajax({
		...
		success:function(e){
			for(var i=0;i<e.length;i++){
				...
				var href=e[i].href;
				$("#href").attr("href",href)
	            $("#href > img ").attr("src","http://60.205.105.211/jz/public/upload/images/"+img)
			}
		}
	});
	$.ajax({
		url:"http://60.205.105.211:2403/about?classification=1",
		dataType:"json",
		type:"get", 
		success:function(e){
